...
namespace Insane.UniversalApps.Cryptography
{
    ...
    public class AesEncryptionManager
    {
        ...
        private static byte[] GenerateValidKey(byte[] KeyBytes)
        {
            ...
            {
                HashAlgorithmProvider Provider = HashAlgorithmProvider.OpenAlgorithm(Windows.Security.Cryptography.Core.HashAlgorithmNames.Sha256);
                IBuffer Buffer = CryptographicBuffer.CreateFromByteArray(KeyBytes);
                ...
                byte[] hash = new byte[MAX_KEY_LENGTH];
        ...
        {
            SymmetricKeyAlgorithmProvider AesAlgorithm = SymmetricKeyAlgorithmProvider.OpenAlgorithm(SymmetricAlgorithmNames.AesCbcPkcs7);
            CryptographicKey CryptoKey = AesAlgorithm.CreateSymmetricKey(CryptographicBuffer.CreateFromByteArray(GenerateValidKey(Key)));
            IBuffer IV = CryptographicBuffer.GenerateRandom(MAX_IV_LENGTH);
            IBuffer BufferEncrypted = CryptographicEngine.Encrypt(CryptoKey, CryptographicBuffer.CreateFromByteArray(PlainBytes), IV);
            byte[] Encrypted = new byte[BufferEncrypted.Length];
            CryptographicBuffer.CopyToByteArray(BufferEncrypted, out Encrypted);
            byte[] iv = new byte[MAX_IV_LENGTH];
            CryptographicBuffer.CopyToByteArray(IV, out iv);
            byte[] ret = new byte[Encrypted.Length + MAX_IV_LENGTH];
            Array.Copy(Encrypted, ret, Encrypted.Length);
            Array.Copy(iv, 0, ret, ret.Length - MAX_IV_LENGTH, MAX_IV_LENGTH);
            return ret;
        }
        ...
        /// <param name="Key">Bytesde la llave.</param>
        ...
        public static byte[] DecryptRaw(byte[] CipherBytes, byte[] Key)
        {
            ...
            IBuffer IV = CryptographicBuffer.CreateFromByteArray(iv);
            ...
            IBuffer Decrypted = CryptographicEngine.Decrypt(CryptoKey, CryptographicBuffer.CreateFromByteArray(RealBytes), IV);
            byte[] ret;
